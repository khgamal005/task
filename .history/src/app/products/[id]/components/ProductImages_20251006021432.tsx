...
export default function ProductImages({ images }: ProductImagesProps) {
  ...
  const displayedThumbnails = images.slice(0, 2); // Show only 2 thumbnails
  const remainingImagesCount = images.length - 2; // Calculate remaining after 2
  ...
  return (
    <div className="w-full space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        {/* Remaining Images Overlay (3rd position) */}
        {remainingImagesCount > 0 && (
          <div
            className="relative aspect-square bg-gray-200 rounded-lg overflow-hidden cursor-pointer"
            onClick={() => selectImage(2)} // Select first of remaining images (index 2)
          >
